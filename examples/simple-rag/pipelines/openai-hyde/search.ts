...
// Search options
export interface OpenAIHydeSearchOptions {
  query: string;
  topK?: number;
}

// OpenAI-specific embedding generator (same as regular OpenAI)
const generateEmbedding = async (input: string): Promise<number[]> => {
  try {
    ...
    console.error(
      "\nError generating OpenAI embedding. Check your OpenAI API key.",
    );
    return [];
  }
};
...
// Find similar movies using OpenAI-Hyde embeddings
...
export const searchWithOpenAIHyde = async (
  options: OpenAIHydeSearchOptions,
  ...
  try {
    ...
    if (!exists) {
      console.log(
        "\nOpenAI-Hyde pipeline has not been run yet or has no embeddings. Run: npm run openai-hyde",
      );
      return { results: [], success: false };
    }

    // Generate embedding (using standard OpenAI embedding function for the query)
    ...
    console.error("\nError searching with OpenAI-Hyde:", error);
    return { results: [], success: false };
  }
};
