...
// OpenAI-specific embedding generator
...
// Find similar movies using OpenAI embeddings
...
export const searchWithOpenAI = async (
  ...
  try {
    ...
    if (!exists) {
      console.log(
        "\nOpenAI pipeline has not been run yet or has no embeddings. Run: npm run openai",
      );
    ...
    console.error("\nError searching with OpenAI:", error);
    return { results: [], success: false };
  }
};
